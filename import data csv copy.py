...
def import_from_csv(particular_stock_csv):
	...
	with open(str(file_location_csv)) as csvfile:														#Importing the csv file into a variable
		...
		for row in read_csv:																			#Reading each row of .csv and append it
			row[1] = float(row[1])
			row[2] = float(row[2])
			aggregate_csv[particular_stock_csv].append(row)

particular_stock_csv = raw_input('Tell me the name of the stock that you want to inport: ')			#You can directly input the file name if this program
